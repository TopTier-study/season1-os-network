...
: 버스에서 오류가 발생해 프로세스가 종료, 주소 자체는 존재하지만 접근 방식이 잘못된 경우

- **프로세스가 메모리에서 정렬되지 않은 경우**
    - CPU는 명령어를 word 단위로 읽기 때문에 word의 배수 단위로 정렬되지 않은 경우 예외를 발생시킬 수 있다.
        - 예외 발생 이유 → RISC 아키텍쳐의 특징
    - Swift 코드 레벨에서는 컴파일러가 정렬해주므로 거의 발생하지 않는다.
- **포인터 인증에 실패한 경우**
    - ARM64 CPU는 암호학적 서명이 포함된 포인터 인증 코드를 사용하여 메모리 내 포인터의 예상치 못한 변경을 감지하고 보호한다.
    - 인증에 실패한 경우, SIGBUS와 하위 타입으로 KERN_INVALID_ADDRESS를 지정해 예외를 발생시킨다.

**EXC_BAD_ACCESS (SIGSEGV)**

: 메모리 결함이 발생해 프로세스가 종료, 유효하지 않은 주소에 접근하려는 경우
