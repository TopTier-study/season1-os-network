...
- Spectre/Meltdown 이후 추가 비용
  ...
  - x86에서는 큰 이슈였고, ARM에서는 하드웨어 수준 완화가 있지만 여전히 비용 요인 중 하나입니다.
- 파라미터 검증
...
|    IPC     | 고수준 IPC(소켓, 파이프)  |     저수준 IPC(메시지 패싱)      |
...
ARM64에서 `svc`의 immediate 값(`#0x80`은 커널이 무시하며, 실제 시스템 콜 번호는 x16 레지스터로 전달됩니다.  
...
- `read`
  - 메인 스레드에서 가장 무거운 시스템 콜입니다. (평균 9.38ms)
- `sys_ulock_wait`
  - 다운샘플링 작업이 존재하는데, `CGImageSourceCreateThumbnailAtIndex` 내부에서 vImage가 GCD `dispatch_apply`를 사용해 리사이징을 병렬 처리하는데, 메인 스레드가 그 병렬 작업의 완료를 기다리면서 락 대기하는 비용입니다.
  - 다운샘플링 자체가 내부적으로 백그라운드 워커를 쓰지만, 결과를 기다리는 건 메인 스레드입니다.
- `mmap`
  - `CGImageSource`가 이미지 데이터를 메모리 매핑하는 과정입니다.
- `mach_vm_allocate_trap`
  - 다운샘플링된 비트맵 + 중간 버퍼 할당을 위한 가상 메모리 요청입니다.

| 시스템 콜               | 횟수  | 누적 시간 | Avg     | Max     | 의미                          |
| ----------------------- | ----- | --------- | ------- | ------- | ----------------------------- |
| `sys_ulock_wait`        | 3,126 | 442ms     | 141μs   | 848μs   | 락 대기 (GCD 디스패치 동기화) |
| `mach_msg2_trap`        | 2,580 | 2.00s     | 774μs   | 125ms   | RunLoop 메시지 대기           |
| `open`                  | 26    | 1.63ms    | 62.8μs  | 111μs   | 파일 열기                     |
| `read`                  | 26    | 243ms     | 9.38ms  | 165ms   | 파일 읽기                     |
| `sys_fstat64`           | 26    | 33.71μs   | 1.30μs  | 852ns   | 파일 크기 조회                |
| `sys_close`             | 26    | 173μs     | 6.68μs  | 10.88μs | 파일 닫기                     |
| `lstat64`               | 25    | 1.77ms    | 70.64μs | 156μs   | 파일 경로 메타데이터 조회     |
...
#### [3] 개선 및 Instruments 분석

`open`→`read`→`close`→`디코딩`->`다운샘플링` 작업을 백그라운드에서 작업하도록 수정했습니다.

1️⃣ System Trace - Main Thread

- `read`
  - 문제 버전보다 횟수는 늘었으나, 소요시간이 감소했습니다.
  - 이건 이미지 파일을 읽는 read가 아니라, 시스템 프레임워크가 내부적으로 사용하는 가벼운 read입니다.
  - 이미지 파일의 무거운 read는 전부 백그라운드 스레드로 이동했습니다.
- `open`
  - read와 비슷하게 호출 횟수는 증가했습니다.
  - 이미지 파일의 open이 아니라, 메인 스레드가 자유로워지면서 시스템 프레임워크가 정상적으로 리소스 파일, 설정 파일 등을 접근하는 호출들입니다.

| 시스템 콜     | 횟수 | 누적 시간 | Avg     | Max   | 의미           |
| ------------- | ---- | --------- | ------- | ----- | -------------- |
| `open`        | 647  | 6.54ms    | 10.10μs | 173μs | 파일 열기      |
| `read`        | 195  | 15.92ms   | 81.64μs | 478μs | 파일 읽기      |
...
2️⃣ Hangs  
Hang이 발생하지 않았습니다.  
실제로도 매우 매끄럽게 스크롤이 작동했습니다.

**3️⃣ 결론**

> 아래 `5.의문`에서 고민한 내용이 담긴 결론입니다.  
> 해당 내용을 참고해주세요.

Instruments로 분석을 하기 전에도 문제 버전에는 확실히 스크롤 시 화면이 버벅였고, 개선 후에는 매끄럽게 실행되는 것을 확인했습니다.

**1\. 시스템 콜 비용 자체가 아니라, 어디서 호출하는지가 중요합니다.**  
동일한 코드, 동일한 시스템 콜, 동일한 다운샘플링에 스레드만 바꿨을 뿐인데 Hang이 17번 → 0번으로 사라졌습니다.  
**2\. 시스템 콜은 연쇄 작업의 시작점입니다.**  
`open`→`read`→`close` 는 그 자체로 많은 시간을 쓰진 않지만, 이후 이어지는 디코딩까지 같은 스레드에 묶이게 합니다.  
메인 스레드에서 I/O를 시작하면, 후속 작업 전체가 메인 스레드를 블로킹하는 상황이 발생합니다.  
**3\. 현업에서의 판단 기준**  
어떤 API가 내부적으로 커널을 거치는 작업인지 알면, "이걸 메인 스레드에서 호출해도 되는가?"라는 질문에 즉시 답할 수 있을 듯 합니다.  
`UIImage(contentsOfFile:`) 한 줄도 내부적으로 `open`/`read`/`close`를 수행한다는 것을 아는 것이 핵심!

## 5. 의문

### 시스템 호출을 많이하지만, 실제로 소요 시간이 많이 들지 않는다면 그것도 문제 상황일까?

이미지를 불러와서 디코딩하는 작업을 의도적으로 메인 스레드에 두는 실험의 결과를 보면서 고민했습니다.  
이미지를 불러오는 I/O 작업은 많이 발생한 건 맞지만, 시간을 많이 잡아먹어 메인 스레드를 블로킹 할 정도는 아니었습니다.  
그럼 디코딩 작업(여기에도 시스템 호출은 있지만)이 무거울 뿐이지, 이 상황에서 **I/O 작업이 문제 지점은 아니지 않을까 생각**했습니다.  
이번 문제에서는 **메인 스레드에서 I/O를 시작해서 후속 작업이 전부 메인에 묶이는 것이 핵심**이었습니다.  
디코딩 중에는 임시 이미지를 보여주며 버벅임을 방지할 수는 있으나 이미지가 늦게 그려지는 것 또한 좋지 않은 사용자 경험을 만듭니다.

**그렇다면, 이렇게 영향을 받는 후속 작업이 없으며 문제 없이 실행된다고 느낄 정도의 빠른 작업이라면 어떨까**요?  
첫번째로 확인했던 write 1000번이 그 예시가 될 수 있습니다.  
실제로 Instruments에서 확인하면 대부분의 시스템 콜은 체감하지 못할 정도로 적은 비용이 듭니다.  
**iOS 개발자가 이 개념을 알고 있다는 건, 특정 API의 시스템 호출 비용이 얼마나 들지 계산하는 것보다 어떤 API가 시스템 호출을 거치는 작업임을 인지하고 잠재적인 위험을 고려할 수 있다고 생각을 결론지었습니다.**
