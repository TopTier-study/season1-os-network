...
#### 서로 다른 컴퓨터가 통신하기 위해 프로토콜이 정의하는 요소들 예시
| **요소**             | **설명**   |
|--------------------|----------|
| Data Format        | 메시지 구조   |
| Transmission Order | 전송 순서    |
| Message Boundary   | 메시지 시작/끝 |
| Error Handling     | 오류 처리 방식 |
| State              | 통신 상태    |
...
### 프로토콜의 표준화 (Standardization)
네트워크 프로토콜은 **표준화된 규칙**이다.

네트워크에 연결된 장치들 -> 서로 다른 운영체제, 하드웨어, 소프트웨어 사용 가능
그러나 동일한 프로토콜을 사용하면 통신 가능‼️📞

> 프로토콜은 컴퓨터 간 공통 언어 역할을 한다.
